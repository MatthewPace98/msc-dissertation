...
\item \url{https://chagall.med.cornell.edu/RNASEQcourse/}
\item \url{https://training.galaxyproject.org/training-material/topics/transcriptomics/tutorials/rb-rnaseq/tutorial.html}
...
\item \url{https://chagall.med.cornell.edu/RNASEQcourse/Intro2RNAseq.pdf}
\end{itemize}
...
STAR is an open-source software package that performs local, splice-aware alignment in two major steps: (1) seed search and (2) clustering, stitching and scoring. It was coded in C++ for the specific purpose of mapping RNAseq reads to a genome.
...
Pathview \citep{luo2013pathview}, like GAGE, is a Bioconductor \citep{gentleman2004bioconductor} R package which visualises GAGE results as an image of the chosen KEGG pathway highlighting the differentially expressed genes according to their logFCs. SBGNview \citep{dong2022sbgnview} offers a wider array of species and pathway knowledge-bases if such are desired. It supports two methods for pathway visualisation: the native KEGG view and the third party Graphviz \citep{ellson2001graphviz}. Graphiz provides better control over the graphical nodes and edges, at the cost of certain pathway metadata	 (e.g. cell types, temporal information).

%Include pathview image?


goana edgeR function for GO annotations

Graphical representation: PCA, multidimensional scaling (MDS), Venn diagrams, volcano plots, sashimi plots, mean-difference (MD) plot
Limited by lack of replicates
